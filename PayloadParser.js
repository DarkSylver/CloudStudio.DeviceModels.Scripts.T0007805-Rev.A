...
{
    var payloadData = payload.asBytes();
    var decoded = Decoder(payloadData, payload.port);
    env.log(decoded);
    ...
    if (decoded.barometric_pressure != null) {
        ...
        if (sensor4 != null)
            sensor4.updatePressureSensorStatus(decoded.barometric_pressure);
    };
    
    // Store CO2
    if (decoded.co2_pressure_compensated != null) {
        var sensor5 = device.endpoints.byAddress("5");

        if (sensor5 != null)
            sensor5.updatePpmConcentrationSensorStatus(decoded.co2_pressure_compensated);
    };   
